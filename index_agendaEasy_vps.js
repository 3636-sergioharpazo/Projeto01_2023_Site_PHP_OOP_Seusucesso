...
console.log('ProtocoloPassando:'+codigoAgendamento)
        try {
            const response = await axios.post(
                'https://agendaeasy.shop/consulta_bot_codigo.php',
                JSON.stringify({ protocolo: codigoAgendamento }),
                {
                    headers: {
...
if (msg.body === '2' && msg.from.endsWith('@c.us')) {
    ...
    async function verificarDisponibilidade(id_dentista,id_empresa, data_agendamento) {
        ...
        try {
            // Enviando requisição para o servidor para verificar horários
            
            const response = await axios.post('https://agendaeasy.shop/verificar-horario.php', JSON.stringify({
            ...
            console.log('Resposta da API horários disponíveis:', response.data);
...
if (resposta.toLowerCase() === 'sim') {
    ...
    dataDigitada = new Date(parseInt(ano), parseInt(mes) - 1, parseInt(dia));

    await client.sendMessage(msg.from, `📆 Agendando para hoje: ${data_agendamento}`);
} else {
    await client.sendMessage(msg.from, '✅ Você deseja informar apenas o *dia* (ex: 13) ou digitar a data completa no formato 04/11/2025? (Responda com "Dia" ou "Data Completa")');
    ...
    if (escolhaData.toLowerCase() === 'dia') {
        // Envia o dia e usa o mês e ano atuais
        await client.sendMessage(msg.from, '✅ Envie o *dia* (ex: 13). Usaremos o mês e o ano atuais.');

        let diaInformado = null;
        let tentativas = 0;

        while (!diaInformado && tentativas < 3) {
            const resposta = await solicitarCampo(
                null,
            ...
            if (!resposta) {
                await client.sendMessage(msg.from, '❌ Nenhuma resposta recebida. Por favor, envie o *dia* (ex: 13).');
                tentativas++;
                continue;
            }
            ...
            if (['menu', 'início', 'voltar'].includes(respostaLimpa)) {
                await client.sendMessage(msg.from, '⚠️ Você está em uma etapa específica. Por favor, envie apenas o *dia* (ex: 13).');
                tentativas++;
                continue;
            }

            if (/^\d{1,2}$/.test(respostaLimpa)) {
                diaInformado = respostaLimpa;
                await client.sendMessage(msg.from, `📅 Dia recebido: *${diaInformado}*`);
            } else {
                await client.sendMessage(msg.from, '❌ Dia inválido! Envie apenas o dia (ex: 13).');
                tentativas++;
            }
        }

        if (!diaInformado) {
            await client.sendMessage(msg.from, '⚠️ Muitas tentativas inválidas. Cancelando o agendamento.');
            return;
        }

        data_agendamento = `${String(diaInformado).padStart(2, '0')}/${mes}/${ano}`;
        let partes = data_agendamento.split('/');
        dataDigitada = new Date(parseInt(partes[2]), parseInt(partes[1]) - 1, parseInt(partes[0]));
    } else {
        // Envia a data completa no formato DD/MM/AAAA
    ...
    await client.sendMessage(msg.from, `📆 Agendando para: ${data_agendamento}`);
}

let hojeSemHoras = new Date();
hojeSemHoras.setHours(0, 0, 0, 0);

let dataDigitadaSemHoras = new Date(dataDigitada);
dataDigitadaSemHoras.setHours(0, 0, 0, 0);
...
while (continuarConsultas) {
    ...
    let id_empresa=ID_EMPRESA;
   console.log('Numero empresa dentro while:'+ID_EMPRESA)
    const horariosDisponiveis = await verificarDisponibilidade(id_dentista,id_empresa,data_agendamento);
    if (horariosDisponiveis.length > 0) {
        let mensagem = `✅ *Horários disponíveis para ${data_agendamento}:*\n\n`;
        horariosDisponiveis.forEach(horario => {
            mensagem += `🕒 ${horario}\n\n`;
        });
        mensagem += `*Escolha o seu Horário:* (Formato: ⏰ 10:00 ou 1000)\n\n`;
        ...
        let entrada = await solicitarCampo(
            null,
            '❌ Entrada inválida! Envie no formato 10:00, 1000 ou "Nova Data".',
            /^([01]?\d|2[0-3]):?([0-5]\d)$|^[Nn]ova [Dd]ata$|^\d{1,2}$/,
            'Entrada recebida'
        );
        if (!entrada) return;

        // Caso escolha "Nova Data"
        if (/^[Nn]ova [Dd]ata$/.test(entrada)) {
            ...
            const [dia, mes, ano] = nova_data.split('/').map(Number);
            const dataSelecionada = new Date(ano, mes - 1, dia);
            const hoje = new Date();
            hoje.setHours(0, 0, 0, 0);

            if (dataSelecionada < hoje) {
                await client.sendMessage(msg.from, '❌ Não é possível agendar para uma data anterior à de hoje. Tente novamente.');
                continue;
            }
        ...
        // Caso escolha apenas o dia
        if (/^\d{1,2}$/.test(entrada)) {
            const hoje = new Date();
            const dia = entrada.padStart(2, '0');
            const mes = String(hoje.getMonth() + 1).padStart(2, '0');
            const ano = hoje.getFullYear();
            const dataTentativa = new Date(ano, parseInt(mes) - 1, parseInt(dia));
            hoje.setHours(0, 0, 0, 0);

            if (dataTentativa < hoje) {
                await client.sendMessage(msg.from, '❌ Não é possível agendar para um dia anterior ao de hoje. Tente novamente.');
                continue;
            }
        ...
        // Normaliza horário (formata para HH:MM)
        if (/^\d{3,4}$/.test(entrada)) {
            entrada = entrada.padStart(4, '0'); // Garante 4 dígitos
            entrada = `${entrada.substring(0, 2)}:${entrada.substring(2)}`;
        }
...
await client.sendMessage(msg.from, `📝 *Confirme as informações:*\n\n` +
    ...
    `✅ *Digite "Sim"* para confirmar\n❌ *Digite "Cancelar"* para cancelar e voltar ao menu principal\n📜 *Digite "Menu"* para retornar ao menu principal.\n❓ Para consultar outra data, digite "Nova Data".`);
            ...
            if (resposta.toLowerCase().trim() === 'sim') {
                ...
                try {
                    const protocoloResponse = await axios.post('https://agendaeasy.shop/gerar_protocolo.php', JSON.stringify({
                    ...
                    console.log('Resposta API envio Protocolo:'+protocolo );
                ...
                await client.sendMessage(msg.from, '📅 Envie a nova data para consulta.\n*Data:* (Formato: 📅 04/11/2025)');
                 ...
                 let novosHorarios = await verificarDisponibilidade(id_dentista, id_empresa,data_agendamento);
                         ...
                         if (resposta.toLowerCase().trim() === 'sim') {
                            ...
                            await client.sendMessage(msg.from, '✅ Agendamento confirmado! Obrigado.');
                             ...
                             try {
                                 ...
                                 console.log('retorno API PROTOCLO: '+protocolo);
...
async function obterConfiguracoes() {
    try {
        const idEmpresa = ID_EMPRESA; // ou pegue dinamicamente
